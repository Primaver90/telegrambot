...
def money_amount(maybe_money: dict):
    """
    Gestisce strutture tipo:
    {money:{amount:249.0,currency:'EUR',displayAmount:'249,00€'}}
    oppure {amount:249.0, currency:'EUR'}
    """
    if not isinstance(maybe_money, dict):
        return None
    if "money" in maybe_money and isinstance(maybe_money["money"], dict):
        m = maybe_money["money"]
        return parse_eur_amount(m.get("amount") or m.get("displayAmount"))
    return parse_eur_amount(maybe_money.get("amount") or maybe_money.get("displayAmount"))
...
# CREATORS API
# =========================
# Importante: usare SUMMARIES per il prezzo (più affidabile su SearchItems).
RESOURCES_LEVELS = [
    # Level 1: prezzo + savings (consigliato)
    [
...
def _get_items_array(response_json: dict):
    """
    Alcune risposte sono:
      { "searchResult": { "items": [...] } }
    ma dai tuoi log spesso è:
      { "items": [...], "searchURL": "...", "totalResultCount": ... }
    Qui gestiamo entrambe.
    """
    if not isinstance(response_json, dict):
        return []
    sr = response_json.get("searchResult")
    if isinstance(sr, dict) and isinstance(sr.get("items"), list):
        return sr.get("items") or []
    if isinstance(response_json.get("items"), list):
        return response_json.get("items") or []
    return []
...
def extract_from_item_dict(item: dict):
    ...
    # Se violatesMAP True spesso prezzo non è pubblicabile: skip se compare.
    listings = offersv2.get("listings") or []
    if listings:
        vmap = bool((listings[0] or {}).get("violatesMAP"))
        if vmap:
            return {
                "asin": asin,
                "title": title,
    ...
    # ---- preferisci SUMMARIES (lowestPrice + savings) ----
    ...
    if isinstance(s0, dict):
        lp = s0.get("lowestPrice") or s0.get("lowestprice") or {}
        price_val = money_amount(lp)

        sav = s0.get("savings") or {}
        sav_amount = money_amount(sav)  # a volte savings è money-ish
        sav_pct = sav.get("percentage") if isinstance(sav, dict) else None
        try:
            sav_pct = int(sav_pct) if sav_pct is not None else None
        except:
            sav_pct = None

        if price_val is not None:
            if sav_amount is not None and sav_amount > 0:
                old_val = price_val + sav_amount
            elif sav_pct is not None and sav_pct > 0:
                try:
                    old_val = price_val / (1 - (sav_pct / 100.0))
                except:
                    old_val = None

    # ---- fallback LISTINGS.price se summaries non c'è ----
    if price_val is None and listings:
        l0 = listings[0] or {}
        price_obj = l0.get("price") or {}
        price_val = money_amount(price_obj)
    ...
    # calcolo discount
    if old_val and old_val > 0 and old_val >= price_val:
        try:
