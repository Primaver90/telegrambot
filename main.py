...
# Creators API credentials (Associates Central -> Creators API)
...
# Token endpoint regionale (dal pdf/guida, per IT tipicamente eu-south-2)
# Esempio: https://creatorsapi.auth.eu-south-2.amazoncognito.com/oauth2/token
...
GETITEMS_FALLBACK_MAX = int(os.environ.get("GETITEMS_FALLBACK_MAX", "4"))  # non usato qui, tenuto per compatibilità log
...
# Resources: devono essere coerenti con Creators API dot notation (come da pdf)
# Qui teniamo minimale + prezzo + savingBasis, per evitare 400 "failed to satisfy constraint"
CREATORS_RESOURCES = [
    "images.primary.large",
    "itemInfo.title",
    "offersV2.listings.price",
    "offersV2.listings.savingBasis",
    # se vuoi provare a tirar fuori percentuali quando disponibili:
...
def parse_eur_amount(value):
    ...
    Accetta:
    - float/int (es. 249.0)
    - stringhe tipo "249,00€", "5,99 €", "1.299,00"
    """
    if value is None:
        return None
...
def creators_search_items(keyword: str, page: int):
    ...
    headers = {
        # IMPORTANT: header esatto come da guida: "Bearer <token>, Version <version>"
...
def extract_from_item_dict(item: dict):
    """
    Estrae asin, titolo, url, img, price_new, price_old, discount
    leggendo il formato visto nei tuoi log:
    offersV2.listings[0].price.money.{amount/displayAmount}
    offersV2.listings[0].savingBasis.money.{amount/displayAmount}
    """
...
def _first_valid_item_for_keyword(kw, pubblicati):
    ...
    for page in range(1, PAGES + 1):
        try:
            ...
            # log raw keys
            if DEBUG_AMAZON:
                _debug(f"kw={kw} page={page} keys={list(data.keys())}")

            # struttura osservata nei tuoi log: data["searchResult"]["items"]
        ...
        for item in items:
            ...
            # se manca prezzo, skip
            if price_val is None:
                reasons["no_price"] += 1
                if DEBUG_AMAZON:
                    _debug(f"asin={asin} no_price | has_offers={bool((item.get('offersV2') or {}).get('listings'))}")
                continue
