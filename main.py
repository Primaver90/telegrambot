...
# ============================================================
SEARCH_RES_LEVELS = [
    # Ricco (spesso rifiuta savingBasis, quindi fallback)
    ["images.primary.large", "itemInfo.title", "offersV2.listings.price", "offersV2.listings.savingBasis"],
    # Medio (a volte rifiuta summaries)
    ["images.primary.large", "itemInfo.title", "offersV2.listings.price", "offersV2.summaries.lowestPrice", "offersV2.summaries.savings"],
    # Minimo (quello che nei tuoi log passa sempre)
...
GET_RES_LEVELS = [
    ["images.primary.large", "itemInfo.title", "offersV2.listings.price", "offersV2.listings.savingBasis", "offersV2.summaries.savings"],
    ["images.primary.large", "itemInfo.title", "offersV2.listings.price", "offersV2.summaries.savings"],
    ["images.primary.large", "itemInfo.title", "offersV2.listings.price"],
    ["images.primary.large", "itemInfo.title"],
]
...
def extract_from_item(item: dict):
    ...
    if l0:
        # prezzo nuovo
        money_amount = safe_get(l0, "price", "money", "amount", default=None)
        display_amount = safe_get(l0, "price", "displayAmount", default=None)
        price_val = parse_eur_amount(money_amount if money_amount is not None else display_amount)

        # prezzo vecchio (savingBasis)
        sb_amount = safe_get(l0, "savingBasis", "money", "amount", default=None)
        sb_display = safe_get(l0, "savingBasis", "displayAmount", default=None)
        old_val = parse_eur_amount(sb_amount if sb_amount is not None else sb_display)

        # discount % se c'è in savings
        perc = safe_get(l0, "price", "savings", "percentage", default=None)
        if perc is None:
            perc = safe_get(l0, "savings", "percentage", default=None)
        if isinstance(perc, (int, float)):
            disc = int(perc)

        # Se non ho perc, lo calcolo da old/new se ho savingBasis
        if disc == 0 and price_val is not None and old_val is not None and old_val > 0:
            try:
                disc = int(round((1.0 - (price_val / old_val)) * 100))
            except:
                disc = 0
